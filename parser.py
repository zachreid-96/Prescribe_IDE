...
def _parse_command(c1):
    ...
    if keyword == "FRPO":
        ...
        if keyword not in list(parameter_list.keys()):
            given_tokens = _generate_tokens(c1)
            expected_tokens = _get_expected_tokens(given_tokens)
            return _compare_tokens(expected_tokens, given_tokens)
    ...
    if format_keys[0] == command_keys[0]:
        ...
        if len(command_keys) == 2:
            return _handle_command_value_check(values[0], parameter_list[parameter]["Values"][0])
...
def _get_option_mutli(indx, int_val, check_value):
    pass

def _handle_multi_conditional_commands(command_value, check_value):
...
def _handle_mutli_unconditional_commands(command_value, check_value):
    _position_count = 0
...
def _handle_command_value_check(command_value, check_value):
    if check_value["Type"] == "Enum":
        ...
        return _handle_range_value_check(command_value, check_value)
    elif check_value["Type"] == "Integer":
        return _handle_integer_value_check(command_value)

    return False, "Unrecognized Command Value Type"

def _handle_range_value_check(command_value, check_value):
    command_int_value = None
    try:
...
def _handle_integer_value_check(command_value):
    command_int_value = None
    try:
        command_int_value = int(command_value)
    except ValueError:
...
def _get_expected_tokens(command):
    parser = Parser()
    ...
    if example_command:
        ...
        tokens = _generate_tokens(new_command)
    #print("get", tokens)
    return tokens

def _compare_tokens(expected, given):
...
def parse_line(full_text):
    ...
    for command in commands:
        success, reason = _parse_command(command)
        if not success:
...
class Parser:
    _instance = None
    _frpo_params_file = r"json/FRPO_parameters.json"
    _prescribe_params_file = r"json/Prescribe_parameters.json"
    _available_commands_file = r"json/available_commands.json"
    _memory = {}
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.frpo_params = _load(cls._frpo_params_file)
            cls._instance.prescribe_params = _load(cls._prescribe_params_file)
            cls._instance.available_commands = _load(cls._available_commands_file)
