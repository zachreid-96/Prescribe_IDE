...
from assistant import Assistant, get_result
...
def _ping_ip(ip):
    ...
    if system == 'windows':
        output = subprocess.check_output(['ping', '-n', '1', ip]).decode('utf-8')
    else:
...
async def _send_lpr_command(ip, command):
    system = platform.system().lower()
    if system == 'windows':
        try:
            ...
            #print("Error", "LPR command not found")
            return "Error", "LPR command not found"
    else:
        try:
            proc = await asyncio.create_subprocess_exec(
                'which', 'lpr',
    ...
    _result = await asyncio.create_subprocess_exec(
            'lpr', '-S', ip, '-P', '9100', command,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE)
...
class PrescribeIDE(ct.CTk):
    def __init__(self):
        ...
        self._geometry = _get_current_screen_geometry()
        ...
        ## Setting up Side Pane for Search Pane and Error Pane
        ...
        ## Setting up Search Pane for Suggestions
        ...
        ## Setting up a container pane for LPR pane and AI Assistant pane
        ...
        self.sending_box.pack(expand=True, side="left", fill="both")#
        ...
        self.ip_entry = ct.CTkEntry(self.sending_box, textvariable=self.ip_text_var, corner_radius=0, width=int(self.x * 0.32)) # int(self.x * 0.75 * 0.94)
        ...
        self.sending_status = ct.CTkLabel(self.sending_box, corner_radius=0, text="Sending Status: inactive")
        self.sending_status.grid(row=3, column=0, sticky="nw", padx=(10, 0), pady=(0, 10), columnspan=2)

        self.send_file = ct.CTkButton(self.sending_box, corner_radius=2, text="Send")
        self.send_file.grid(row=4, column=0, sticky="nw", padx=(10, 0), pady=(0, 10), columnspan=2)
        self.send_file.configure(state="disabled", command=self.send_via_lpr)

        ## Setting up the AI Assistant pane

        self.assistant_separator = ct.CTkFrame(self.codebox_frame, corner_radius=0)
        self.assistant_separator.configure(width=4, fg_color=self.separator_fg_color)
        self.assistant_separator.pack(expand=False, side="left", fill="y")

        self.sending_box_height = int(self.y * 0.75)
        self.assistant_box = ct.CTkFrame(self.codebox_frame, corner_radius=0, height=self.sending_box_height)
        self.assistant_box.configure(fg_color="#282A36")
        self.assistant_box.pack(expand=True, side="right", fill="both")
        self.assistant_box.grid_propagate(False)

        self.assistant_label = ct.CTkLabel(self.assistant_box, corner_radius=0, text="- - - AI Assistant - - -")
        self.assistant_label.grid(row=0, column=0, sticky="nw", padx=(10, 0), pady=(0, 0), columnspan=2)

        self.assistant_prompt = ct.CTkEntry(self.assistant_box, corner_radius=0, width=int(self.x * 0.32))
        self.assistant_prompt.grid(row=1, column=0, sticky="nw", padx=(10, 10), pady=(10, 0), columnspan=5)

        self.assistant_prompt_note = ct.CTkLabel(self.assistant_box, corner_radius=0, text="*  Separate commands with a semi-colon  *")
        self.assistant_prompt_note.grid(row=2, column=0, sticky="nw", padx=(10, 0), pady=(0, 0), columnspan=2)

        self.assistant_process_button = ct.CTkButton(self.assistant_box, corner_radius=2, text="Process Request")
        self.assistant_process_button.grid(row=3, column=0, sticky="nw", padx=(10, 10), pady=(10, 0), columnspan=2)
        self.assistant_process_button.configure(command=self.thread_process_request)

        self.assistant_output = ct.CTkTextbox(self.assistant_box, corner_radius=0)
        self.assistant_output.insert("0.0", "- - - Assistant Output - - -")
        self.assistant_output.configure(state="disabled", fg_color="#282A36")
        self.assistant_output.configure(fg_color="#282A36")
        self.assistant_output.grid(row=4, column=0, sticky="nw", padx=(10, 10), pady=(10, 10), columnspan=2, rowspan=4)

        ## Setting up of various variables to be used later
        ...
        self.custom_popup_save = None
        self.custom_popup_load = None
        self._assistant = None

    def thread_load_assistant(self):
        _thread = threading.Thread(target=self.load_assistant)
        _thread.start()
    ...
    def thread_process_request(self):
        _thread = threading.Thread(target=self.process_request)
        _thread.start()

    def process_request(self):
        _tokens = self.assistant_prompt.get()
        _result = get_result(_tokens)

        self.assistant_output.configure(state="normal")
        if isinstance(_result, str):
            self.assistant_output.delete("3.0", "end")
            self.assistant_output.insert("3.0", _result)
        elif isinstance(_result, list):
            self.assistant_output.delete("3.0", "end")
            self.assistant_output.insert("3.0", f"\n\n{'\n'.join(_result)}")
        self.assistant_output.configure(state="disabled")

    def send_via_lpr(self):
        if self.current_file is None:
            self.custom_popup = ct.CTkToplevel()
            self.custom_popup.title("LPR Status")
            self.custom_popup.geometry("300x150")
            self.custom_popup.resizable(False, False)
            self.custom_popup.transient()
            self.custom_popup.grab_set()

            custom_popup_msg = ct.CTkLabel(self.custom_popup, text="Please load file or save current file")
            custom_popup_msg.pack(expand=False, fill="both")

            self.custom_popup_save = ct.CTkButton(self.custom_popup, corner_radius=0, text="Save and Send")
            self.custom_popup_save.configure(command=self.save_and_send_lpr)
            self.custom_popup_save.pack(expand=False)
            self.custom_popup_load = ct.CTkButton(self.custom_popup, corner_radius=0, text="Load and Send")
            self.custom_popup_load.configure(command=self.load_and_send_lrp)
            self.custom_popup_load.pack(expand=False)

            self.custom_popup.protocol("WM_DELETE_WINDOW", self.on_custom_popup_close)
            self.custom_popup.wait_window(self.custom_popup)
        else:
            self.send_file_via_lpr()

    def send_file_via_lpr(self):

        self.sending_status.configure(text="Sending Status: Sending file...")
        self.update_idletasks()

        _failed_message = ""

        ip_range = list(filter(None, self.ip_entry.get().split(";")))
        for ip in ip_range:
            if not _ping_ip(ip):
                _failed_message = f"{_failed_message}IP {ip} failed to ping.\n"
                continue
            _result, _reason = asyncio.run(_send_lpr_command(ip.strip(), self.current_file))
            if _result == "Error":
                _failed_message = f"{_failed_message}IP {ip} failed due to {_reason}.\n"

        self.sending_status.configure(text="Sending Status: Done")
        self.update_idletasks()

    def on_custom_popup_close(self):
        self.custom_popup.destroy()

    def save_and_send_lpr(self):
        self.custom_popup.destroy()
        self.save_file()

        self.send_file_via_lpr()

    def load_and_send_lrp(self):
        self.custom_popup.destroy()
        self.open_file()
        ...
        for pos, val in enumerate(values):
            for option in param_args["Values"]:
                if ((option.get("Position", -1) == (pos + 2) and option.get("Type", "") == "enum" and
                        option.get("Parent Position", -1) == (pos + 1) and str(option.get("Parent Option", -1)) == val) or
                    (option.get("Position", -1) == (pos + 2) and option.get("Type", "") == "range" and
    ...
    def format_options(self, param_args, val, position):
        current_frpo_arg = ""
        if param_args.get("Multi-Parameter", 0) == 1:
        ...
        if self.current_param in param.keys():
            ...
            for val in param_args.get("Values"):
                if param_args.get("Multi-Parameter", 0) == 1:
                    ...
                    self.tab_width *= 2
                if val["Type"] == "Integer" or val["Type"] == "String":
                    current_frpo_arg += f"{'':>{self.tab_width}}{val.get('Comment')}\n"
                elif val["Type"] == "Enum":
                    for opt_key, opt_val in val["Options"].items():
                        current_frpo_arg += f"{'':>{self.tab_width}}{opt_key}: {opt_val}\n"
...
if __name__ == "__main__":
    app = PrescribeIDE()
    app.after_idle(app.thread_load_assistant)
    app.mainloop()
